    ...
    <div className="bg-primary-bg">
      <div className="mx-auto max-w-7xl px-4 pt-10 pb-20 md:pt-5 md:pb-20">
        ...
        <div className="relative">
          <div ref={scrollContainerRef} className="overflow-hidden md:block hidden">
              ...
              {infiniteTours.map((tour, index) => (
                <div key={index} className="flex-shrink-0 bg-white overflow-hidden w-[610px]">
                  ...
                  <div className="p-6 max-w-[610px]">
                    <h3 className="text-2xl font-bold text-brand-navy mb-3" style={{ fontFamily: '"Gilroy", Sans-serif', fontWeight: 800 }}>
                      {tour.title}
                    </h3>
                    <p className="text-brand-deep mb-4 leading-relaxed" style={{ fontFamily: '"Asap", Sans-serif' }}>
                      {tour.description}
                    </p>
                    <div className="flex items-center justify-between gap-2">
                      <span className={`${tour.textColor} font-bold text-lg`}>{tour.distance}</span>
                      <div className="flex gap-2">
                        <Link href={tour.route || `/tour/${tour.id}`} className="bg-gray-100 hover:bg-gray-200 text-brand-navy px-4 py-2 text-sm font-semibold transition-colors duration-300">
                          Learn More
                        </Link>
                        <Link href={`/booking?tour=${tour.bookingId}`} className={`${tour.buttonColor} text-white px-4 py-2 text-sm font-semibold transition-colors duration-300`}>
                          Book Now
                        </Link>
                      </div>
                    </div>
